...
class KafkaKafkaResourceIT {
    ...
    void setup() {
        ...
        KafkaKafkaResource kafkaResource = new KafkaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/kafka-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/kafka-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
